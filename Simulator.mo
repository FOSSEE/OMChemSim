  ...
  extends General_Properties(
    ...
    package Thermodynamic_Packages
      model Raoults_Law
        ...
        Real K[NOC](each min = 0), gamma[NOC], gammaBubl[NOC], gammaDew[NOC], resMolSpHeat[3], resMolEnth[3], resMolEntr[3];
      equation
        for i in 1:NOC loop
          K[i] = Psat(comp[i].VP, T) / P;
        end for;
        gamma[:] = fill(1, NOC);
        gammaBubl[:] = fill(1, NOC);  
        gammaDew[:] = fill(1, NOC);
...
equation
  gamma = Gamma.gamma;
  for i in 1:NOC loop
    dewLiqMolFrac[i] = compMolFrac[1, i] * Pdew / (gammaDew[i] * Psat(comp[i].VP, T));
...
//Bubble point calculation
  Pbubl = sum(gammaBubl[:] .* compMolFrac[1, :] .* exp(comp[:].VP[2] + comp[:].VP[3] / T + comp[:].VP[4] * log(T) + comp[:].VP[5] .* T .^ comp[:].VP[6]));
//Dew point calculation
  Pdew = 1 / sum(compMolFrac[1, :] ./ (gammaDew[:] .* exp(comp[:].VP[2] + comp[:].VP[3] / T + comp[:].VP[4] * log(T) + comp[:].VP[5] .* T .^ comp[:].VP[6])));
  if P >= Pbubl then
//below bubble point region
    compMolFrac[3, :] = zeros(NOC);
    sum(compMolFrac[2, :]) = 1;
