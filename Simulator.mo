...
//ideal flash
  ...
  package PF_Reactor
    model PFR //Plug Flow Reactor
          ...
          Real T "Inlet Temperature";
          Real P "Inlet pressure";
          //Component Molar Flow rates of respective phases
          Real compMolFlow[3, NOC](each start = 50);
          //Total molar flow rates of respective phases
          Real totMolFlow[3] "Total inlet molar flow rate";
          //Mole Fraction of components in respective phases
          Real compMolFrac[3, NOC](each start = 0.5) "Mole Fraction of components in inlet stream";
          Real Enth, Entr, Vapfrac;
          //Output Variables-Connectors
          Real Tout(start = 360.827) "Temperature for which calculations are made";
          Real Pout "outlet pressure in Pa";
          //Total molar flow rates of respective phases in the outlet streams
          Real outTotMolFlow[3](each start = 50) "Total Outlet Molar Flow Rate";
          Real outCompMolFlow[3, NOC](each start = 50) "Component outlet molar flow rate";
          Real outCompMolFrac[3, NOC](each start = 0.5) "Mole Fraction of components in outlet stream";
          Real outEnth, outEntr, outVapfrac;
          //Phase-Equilibria
          Real Pdew(unit = "Pa", start = max(comp[:].Pc));
          Real Pbubl(min = 0, unit = "Pa", start = min(comp[:].Pc));
          Real Beta(start = 0.5);
          //Phase-Equilibria-Outlet Stream
          Real Poutdew(unit = "Pa", start = max(comp[:].Pc));
          ...
          Real X[NOC] "Conversion of the reaction components";
