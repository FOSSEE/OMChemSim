within Simulator.Files.Thermodynamic_Packages;
  ...
  model UNIQUAC
    ...
    parameter Real a[Nc, Nc] = Thermodynamic_Functions.BIP_UNIQUAC(Nc, C.name);
...
//Function :Tow_UNIQUAC is used to instantiated
    tow = Simulator.Files.Thermodynamic_Functions.Tow_UNIQUAC(Nc, a, T);
...
//Calculation of Saturated vapour pressure and Density at the given input condition
    for i in 1:Nc loop
      ...
      Density[i] = Simulator.Files.Thermodynamic_Functions.Dens(C[i].LiqDen, C[i].Tc, T, P) * 1E-3;
    end for;
...
//Function :Poynting_CF is called from the Simulator Package
  for i in 1:Nc loop
    PCF_c[i] = Thermodynamic_Functions.PoyntingCF(Nc,C[i].Pc,C[i].Tc,C[i].Racketparam,C[i].AF,C[i].MW, T, P, gma_c[i], Pvap_c[i], Density[i]);
    PCFbubl_c[i] = Thermodynamic_Functions.PoyntingCF(Nc,C[i].Pc,C[i].Tc,C[i].Racketparam,C[i].AF,C[i].MW, T, Pbubl, gma_c[i], Pvap_c[i], Density[i]);
    PCFdew_c[i] = Thermodynamic_Functions.PoyntingCF(Nc,C[i].Pc,C[i].Tc,C[i].Racketparam,C[i].AF, C[i].MW, T, Pdew, gma_c[i], Pvap_c[i], Density[i]);
  end for;
