within Simulator.Unit_Operations.Distillation_Column;
  ...
  model Reb
   ...
   Real x_pc[3, Nc](each min = 0, each max = 1, each start = 1/(Nc + 1)), Pdew(min = 0, start = sum(C[:].Pc)/Nc), Pbubl(min = 0, start = sum(C[:].Pc)/Nc);
    ...
    replaceable Simulator.Files.Connection.matConn In_Dmy(Nc = Nc, P = 0, T = 0, mixMolFrac = zeros(3, Nc), mixMolFlo = 0, mixMolEnth = 0, mixMolEntr = 0, vapPhasMolFrac = 0) if not Bin annotation(
      Placement(visible = true, transformation(origin = {-100, 0}, extent = {{-10, -10}, {10, 10}}, rotation = 0), iconTransformation(origin = {-100, 0}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
...
//Bubble point calculation
    Pbubl = sum(gmabubl_c[:] .* x_pc[1, :] .* exp(C[:].VP[2] + C[:].VP[3] / T + C[:].VP[4] * log(T) + C[:].VP[5] .* T .^ C[:].VP[6]) ./ liqfugcoeff_bubl[:]);
...
//    sum(xvapout_c[:]) = 1;
    sum(xout_c[:]) = 1;
    for i in 1:Nc loop
      Hvapout_c[i] = Simulator.Files.Thermodynamic_Functions.HVapId(C[i].SH, C[i].VapCp, C[i].HOV, C[i].Tc, T);
    end for;
